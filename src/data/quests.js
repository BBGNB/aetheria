...
export const QUESTS = [
  {
    ...
    type: 'main',
    intro: 'Bren stumbled into Hearthstone bleeding from wolf-bites and gasping about a partner left behind. The road west has gone quiet — except for what hunts on it.',
    steps: [
      { text: 'Hear out the dying scout at Hearthstone\'s gate.',
        when: g => g.flags.has('bren:warned') },
      { text: 'Speak with Elder Vorrin about the pack moving south.',
        when: g => g.flags.has('vorrin:spoken') },
      { text: 'Cross the Lower Meadow — clear the path north.',
        when: g => g.defeatedEnemies.has('meadow:pair1') },
      { text: 'Rescue Cal in Brookside Grove.',
        when: g => g.flags.has('cal:rescued') },
      { text: 'Break the pack\'s grip on the Cave Approach.',
        when: g => g.defeatedEnemies.has('meadow:packTrio') },
      { text: 'Break the Pack Alpha at the Echoing Hollow\'s mouth.',
